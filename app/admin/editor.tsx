...
function PhotoCropper({ source, onCrop }: { source: string; onCrop: (photo: string) => void }) {
  ...
  const move = (event: PointerEvent<HTMLDivElement>) => {
    ...
    setPosition({ x: event.clientX - dragging.current.x, y: event.clientY - dragging.current.y });
  };
  ...
  const crop = () => {
    ...
    image.onload = () => {
      ...
      context.drawImage(image, size / 2 - image.width * scale / 2 + position.x, size / 2 - image.height * scale / 2 + position.y, image.width * scale, image.height * scale);
      onCrop(canvas.toDataURL("image/jpeg", 0.88));
    };
    image.src = source;
  };

  return <div className="admin-photo-cropper"><div className="admin-photo-crop-area" onPointerDown={event => { dragging.current = { x: event.clientX - position.x, y: event.clientY - position.y }; event.currentTarget.setPointerCapture(event.pointerId); }} onPointerMove={move} onPointerUp={stopDragging} onPointerCancel={stopDragging}><img src={source} alt="Escolha o recorte da foto" style={{ transform: `translate(${position.x}px, ${position.y}px) scale(${zoom})` }} /><span className="admin-photo-crop-guide">Arraste para escolher o recorte</span></div><label className="admin-crop-zoom">Zoom<input type="range" min="1" max="3" step="0.05" value={zoom} onChange={event => setZoom(Number(event.target.value))} /></label><button className="button button-outline" type="button" onClick={crop}>Aplicar recorte</button></div>;
}
...
export default function AdminEditor({ initial, available }: { initial: ClubContent; available: boolean }) {
  ...
  const [cropSources, setCropSources] = useState<Record<number, string>>({});
  ...
  async function uploadPhoto(id: number, file: File | undefined) {
    ...
    reader.onload = () => {
      ...
      setCropSources(v => ({ ...v, [id]: reader.result as string }));
