...
export default function AdminEditor({ initial, available }: { initial: ClubContent; available: boolean }) {
  ...
  const addProduct = () => { const id = `peca-${Date.now()}`; setContent(c => ({ ...c, products: [...c.products, { id, name: "Nova peça", edition: "01", label: "Nova coleção", type: "Camiseta", cut: "Básico", photo: "" }] })); setDirty(true); setFeedback(""); };
