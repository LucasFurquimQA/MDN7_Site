...
export default function AdminEditor({ initial, available, instagramConnected }: { initial: ClubContent; available: boolean; instagramConnected: boolean }) {
  ...
  async function uploadPhoto(id: number, file: File | undefined) {
    ...
    if (!file.type.startsWith("image/")) {
      setImports(v => ({ ...v, [id]: { loading: false, message: "Selecione um arquivo de imagem válido." } }));
      return;
    }
    if (file.size > 1_500_000) {
      setImports(v => ({ ...v, [id]: { loading: false, message: "A foto deve ter no máximo 1,5 MB." } }));
      return;
    }
    const reader = new FileReader();
    reader.onload = () => {
      if (typeof reader.result !== "string") {
        setImports(v => ({ ...v, [id]: { loading: false, message: "Não foi possível ler essa foto." } }));
        return;
      }
      updateMember(id, { photo: reader.result });
      setImports(v => ({ ...v, [id]: { loading: false, message: "Foto carregada. Salve as alterações para publicar." } }));
    };
    reader.onerror = () => setImports(v => ({ ...v, [id]: { loading: false, message: "Não foi possível ler essa foto." } }));
  ...
  async function importProfile(id: number, link: string, automatic = false) {
    ...
    try {
      ...
      const data = await response.json() as { error?: string; profile?: Pick<Member, "username" | "name" | "bio"> };
      ...
      updateMember(id, { ...data.profile, photo: currentContent.current.members.find(m => m.id === id)?.photo || "", instagram: instagramUrl(link) });
