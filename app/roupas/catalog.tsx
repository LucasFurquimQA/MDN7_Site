...
function ProductDetails({ product, type, cut, onClose }: { product: Product | null; type: string; cut: string; onClose: () => void }) {
  ...
  return <Dialog open={!!product} onOpenChange={open => { if (!open) onClose(); }}><DialogContent className="product-dialog" showCloseButton={false}><DialogClose className="dialog-close icon-button" aria-label="Fechar detalhes"><X size={22} /></DialogClose>{product && selectedVariant && <><div className="detail-visual">{images.length > 1 && <span className="photo-counter mono">{String(index + 1).padStart(2, "0")} / {String(images.length).padStart(2, "0")}</span>}<Shirt product={product} cut={selectedCut} photo={images[index]} priority />{images.length > 1 && <div className="detail-photo-arrows"><button type="button" className="icon-button" aria-label="Foto anterior" onClick={() => goTo(index - 1)}><ArrowLeft size={18} /></button><button type="button" className="icon-button" aria-label="Próxima foto" onClick={() => goTo(index + 1)}><ArrowRight size={18} /></button></div>}{images.length > 1 && <div className="product-carousel-controls detail-photo-dots" aria-label={`Fotos de ${product.name}`}>{images.map((image, imageIndex) => <button type="button" key={`${imageIndex}-${image.slice(-12)}`} className={imageIndex === index ? "active" : ""} aria-label={`Mostrar foto ${imageIndex + 1} de ${images.length}`} aria-pressed={imageIndex === index} onClick={() => setIndex(imageIndex)} />)}</div>}</div><div className="detail-info"><span className="eyebrow">MIDNIGH7 WEAR / {selectedType.toUpperCase()} / EDIÇÃO {product.edition}</span><DialogTitle>{product.name}</DialogTitle><DialogDescription>Uma paixão que também se veste. {product.label}. Consulte os tamanhos e condições do lote pelo WhatsApp.</DialogDescription><span className="field-title">TIPO DE PEÇA</span><div className="fit-tabs variant-tabs">{variants.map(variant => <button className={`variant-tab ${variant.type === selectedType ? "selected" : ""}`} type="button" key={variant.type} onClick={() => { setSelectedType(variant.type); setSelectedCut(variant.cuts[0]); }}>{variant.type}</button>)}</div><span className="field-title">CORTE DISPONÍVEL</span><div className="fit-tabs variant-tabs">{selectedVariant.cuts.map(availableCut => <button className={`variant-tab ${availableCut === selectedCut ? "selected" : ""}`} type="button" key={availableCut} onClick={() => setSelectedCut(availableCut)}>{availableCut}</button>)}</div><div className="detail-note"><span className="gold-label">VENDA POR LOTE</span><p>As peças são produzidas em lotes. Fale com a gente para reservar a sua.</p></div><a className="button button-gold" href={whatsappUrl(product)} target="_blank" rel="noopener noreferrer"><ShoppingBag size={19} />Pedir pelo WhatsApp<ArrowUpRight size={19} /></a><p className="mockup-note">Imagens de referência das estampas.</p></div></>}</DialogContent></Dialog>;
}
