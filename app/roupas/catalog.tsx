...
function ProductCarousel({ product, type, cut, priority = false, onOpen }: { product: Product; type: string; cut: string; priority?: boolean; onOpen: () => void }) {
    ...
    return <div className="product-carousel"><button type="button" className="product-carousel-image" onClick={onOpen} aria-label={`Ver detalhes de ${product.name}`}><div className="product-tags"><span className="mono">ED. {product.edition}</span><span className="mono">{current ? `${current.view === "piece" ? "PEÇA" : "MODELO"} / ${current.side === "front" ? "FRENTE" : "VERSO"}` : type.toUpperCase()}</span></div><Shirt product={product} cut={cut} photos={photosForVariant(product, type, cut)} side={current?.side || "back"} view={current?.view || "piece"} priority={priority} /><span className="view-product">Ver detalhes<ArrowUpRight size={17} /></span></button>{images.length > 1 && <div className="product-carousel-controls" aria-label={`Fotos de ${product.name}`}>{images.map((image, imageIndex) => <button type="button" key={image.key} className={imageIndex === index ? "active" : ""} aria-label={`Mostrar foto ${imageIndex + 1} de ${images.length}`} aria-pressed={imageIndex === index} onClick={() => setIndex(imageIndex)} />)}</div>}</div>;
  }
