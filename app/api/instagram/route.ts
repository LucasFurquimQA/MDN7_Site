...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!token || !id) return noStoreJson({ username, error: "O @ foi identificado. A importação de nome e bio ainda precisa de uma conexão autorizada com a Meta. Escolha a foto no seu computador." }, 503);
    ...
    api.searchParams.set("fields", `business_discovery.username(${username}){username,name,biography}`);
    const result = await fetch(api, { headers: { Authorization: `Bearer ${token}` }, signal: AbortSignal.timeout(10000) });
    if (!result.ok) return noStoreJson({ username, error: "A Meta não disponibilizou esse perfil. Confira a conexão e as permissões; contas pessoais ou privadas podem não ser compatíveis. Os campos manuais continuam disponíveis, incluindo a foto do computador." }, 422);
    const data = await result.json() as { business_discovery?: { username?: string; name?: string; biography?: string } };
    ...
    return noStoreJson({ profile: { username, name: (profile.name || "").slice(0, 100), bio: (profile.biography || "").slice(0, 400) } });
  } catch (error) {
