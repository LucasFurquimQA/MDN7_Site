...
export async function getChatGPTUser(): Promise<ChatGPTUser | null> {
  ...
  if (userId && email) {
    return readUser(userId, email, requestHeaders);
  }

  const accessEmail = requestHeaders.get(ACCESS_EMAIL_HEADER)?.trim();
  if (accessEmail) {
    return {
      userId: `cloudflare-access:${accessEmail.toLowerCase()}`,
