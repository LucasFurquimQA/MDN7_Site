...
A história, o Instagram oficial, o catálogo de roupas e os sete administradores são gravados em D1. No painel em `/admin`, o responsável também pode editar nomes, tipos, cortes (como oversized, babylook ou básico), edições, legendas e fotos das peças, além de adicionar ou remover itens. As fotos são convertidas para o conteúdo salvo (até 1,5 MB por arquivo). O painel só permite alterações pelo proprietário identificado por `ADMIN_EMAIL`, configurado como variável protegida na hospedagem. Visitantes não possuem acesso de escrita.
